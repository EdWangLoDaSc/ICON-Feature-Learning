...
def build_out_mask(cond_len_list, qoi_kv_len_list, qoi_k_len_list, num_range):
  ...
  for i in range(num):
    ...
    cursor += pair_size
  return np.array(out_mask, dtype=bool)
...
def build_data_sequence(data, cond_bool_list, qoi_kv_bool_list, qoi_k_bool_list, data_quest_qoi_v = None):
  ...
  '''
  # Handle demo data
  demo_cond = np.concatenate([data['demo_cond_k'], data['demo_cond_v']], axis=-1)  # [demo_num, demo_cond_len, k_dim + v_dim]

  demo_qoi_kv = np.concatenate([data['demo_qoi_k'], data['demo_qoi_v']], axis=-1)  # [demo_num, demo_qoi_len, k_dim + v_dim]

  demo_qoi_k = np.pad(data['demo_qoi_k'], ((0, 0), (0, 0), (0, data['demo_qoi_v'].shape[-1])), mode='constant', constant_values=0)  # [demo_num, demo_qoi_len, k_dim + v_dim]

  # Handle quest data
  if 'data_quest_qoi_v' not in data:
    data['data_quest_qoi_v'] = np.zeros((data['quest_qoi_k'].shape[0], data['quest_qoi_k'].shape[1], data['demo_qoi_v'].shape[-1]))

  quest_cond = np.concatenate([data['quest_cond_k'], data['quest_cond_v']], axis=-1)  # [quest_num, quest_cond_len, k_dim + v_dim]
  quest_qoi_kv = np.concatenate([data['quest_qoi_k'], data['data_quest_qoi_v']], axis=-1)  # [quest_num, quest_qoi_len, k_dim + v_dim]
  quest_qoi_k = np.pad(data['quest_qoi_k'], ((0, 0), (0, 0), (0, data['data_quest_qoi_v'].shape[-1])), mode='constant', constant_values=0)  # [quest_num, quest_qoi_len, k_dim + v_dim]
  ...
  sequence = np.concatenate(sequence, axis = 0) # [sequence length, k_dim + v_dim]
...
def build_data_mask(data, cond_bool_list, qoi_kv_bool_list, qoi_k_bool_list):
  ...
  mask = np.concatenate(mask, axis = 0) # [sequence length]
  return mask
...
def plot_model_consts(basic_mask = None, index_matrix = None, out_mask = None):
  ...
  if basic_mask is not None:
    ...
    plt.colorbar()
    assert np.allclose((basic_mask @ basic_mask).astype(bool), basic_mask.astype(bool))
